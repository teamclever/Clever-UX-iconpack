...
import com.clever.ux.util.RequestLimiter;
...
public class RequestsAdapter extends DragSelectRecyclerViewAdapter<RequestsAdapter.RequestVH> {
  ...
  public RequestVH onCreateViewHolder(ViewGroup parent, int viewType) {
    View view =
        LayoutInflater.from(parent.getContext())
            .inflate(
                viewType == 1 ? com.clever.ux.R.layout.list_item_header : com.clever.ux.R.layout.list_item_iconrequest,
                parent,
                false);
  ...
  public void onBindViewHolder(RequestVH holder, int position) {
    ...
    if (position == 0) {
      ...
      if (mAllowRequest == RequestLimiter.WAIT) {
        final String msg =
            c.getString(com.clever.ux.R.string.request_limited, RequestLimiter.get(c).remainingIntervalString());
        holder.title.setText(msg);
      } else if (mAllowRequest == RequestLimiter.NO_LIMIT) {
        holder.title.setText(com.clever.ux.R.string.tap_to_select_app);
      } else {
        holder.title.setText(
            c.getResources().getString(com.clever.ux.R.string.tap_to_select_app_withremaining, mAllowRequest));
      }
      final int bgColor =
          DialogUtils.resolveColor(holder.itemView.getContext(), com.clever.ux.R.attr.window_background_cards);
  ...
  public class RequestVH extends RecyclerView.ViewHolder
    ...
    public RequestVH(View itemView) {
      super(itemView);
      card = ButterKnife.findById(itemView, com.clever.ux.R.id.card);
      title = ButterKnife.findById(itemView, com.clever.ux.R.id.title);
      image = ButterKnife.findById(itemView, com.clever.ux.R.id.image);
      if (card != null) {
